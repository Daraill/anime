...
export const fetchInfoZoro = async (id: string): Promise<ZoroInfo | null> => {
  ...
  try {
    ...
    const animeInfo: ZoroInfo = {
      ...
      jptitle:
        $('.item-title:contains("Japanese") .name').text().trim() || undefined,
      synonyms: [
        $(".dynamic-name").first().text().trim(),
        $('.item-title:contains("Synonyms") .name').text().trim(),
      ].filter(Boolean),
      description: $('.item.item-title:contains("Overview") .text')
        .text()
        .trim(),
      poster: $(".film-poster-img").attr("src") || undefined,
      aired: {
        date_aired:
          $('.item-title:contains("Aired") .name').text().trim() || undefined,
        premiered:
          $('.item-title:contains("Premiered") .name').text().trim() ||
          undefined,
        status:
      ...
      episodes: {
        sub: parseEpisodeCount(
          $filmStats.find(".tick-item.tick-sub").text().trim(),
        ),
        dub: parseEpisodeCount(
          $filmStats.find(".tick-item.tick-dub").text().trim(),
        ),
        total: parseEpisodeCount(
          $filmStats.find(".tick-item.tick-eps").text().trim(),
        ),
      },
      malScore:
        $('.item-title:contains("MAL Score") .name').text().trim() || undefined,
      ...
      seasons: $(".block_area-seasons .os-item")
        .map((i: number, el: any) => ({
          ...
          thumbnail: (
            $(el).find(".season-poster").css("background-image") || ""
          ).replace(/url\((['"])?(.*?)\1\)/gi, "$2"),
        }))
        .get(),
    };
